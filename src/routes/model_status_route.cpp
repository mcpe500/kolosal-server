...
namespace kolosal
{
    ...
    bool ModelStatusRoute::match(const std::string &method, const std::string &path)
    {
        // Match GET /models/{id}/status or GET /v1/models/{id}/status
        std::regex statusPattern(R"(^/(v1/)?models/([^/]+)/status$)");
        return (method == "GET" && std::regex_match(path, statusPattern));
    }

    void ModelStatusRoute::handle(SocketType sock, const std::string &body)
    {
        try
        {
            ...
            ModelStatusErrorResponse defaultErrorResponse;
            defaultErrorResponse.error.message = "Engine status endpoint is available but requires path parameters that aren't currently accessible in this routing system. Use JSON body with model_id for now.";
            ...
            if (!body.empty())
            {
                try
                {
                    ...
                    ModelStatusRequest request;
                    request.from_json(j);

                    if (!request.validate())
                    {
                        ModelStatusErrorResponse errorResponse;
